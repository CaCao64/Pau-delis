...
class DetailsFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        b.btnSeeLine.setOnClickListener {
            ...
            when {
                ...
                matchedLines.size == 1 ->
                    (activity as? MainActivity)?.showFragment(LineDetailFragment.newInstance(matchedLines[0], name), true)
                else -> {
                    ...
                    android.app.AlertDialog.Builder(requireContext())
                        ...
                        .setItems(names) { _, idx ->
                            (activity as? MainActivity)?.showFragment(LineDetailFragment.newInstance(matchedLines[idx], name), true)
                        }
                        .show()
                }
            }
    ...
    private fun passageRow(info: StopInfo): View {
        ...
        header.addView(btnFav)

        // ⟳ refresh
        val btnRefresh = TextView(ctx).apply {
            text = "⟳"
            textSize = 22f
