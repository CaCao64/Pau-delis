...
class TrackingService : Service() {
        ...
        when (intent?.action) {
            ...
            ACTION_START -> {
                ...
                startForeground(notifId, buildNotif(notifId, stopName, lineName,
                    getString(R.string.tracking_starting), 0xFF1565C0.toInt(), R.drawable.ic_bell, hour, minute))

                jobs[notifId]?.cancel()
                jobs[notifId] = scope.launch {
                    trackLoop(notifId, stopName, lineName, hour, minute)
                    jobs.remove(notifId)
            ...
            ACTION_TOGGLE -> {
                ...
                scope.launch {
                    val passage = fetchNextPassage(stopName, lineName)
    ...
    private suspend fun trackLoop(notifId: Int, stopName: String, lineName: String, hour: Int, minute: Int) {
        val nm = getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager

        // Fetch initial
        val first = fetchNextPassage(stopName, lineName)
        val (body0, color0, icon0) = buildContent(first, hour, minute)
        val dest0 = first?.destination ?: ""
        nm.notify(notifId, buildNotif(notifId, stopName, lineName,
        ...
        while (currentCoroutineContext().isActive) {
            ...
            val updated = fetchNextPassage(stopName, lineName)
            ...
            if (hasPassed) {
                ...
                nm.notify(notifId, buildNotif(notifId, stopName, lineName,
                    getString(R.string.tracking_passed).format(origHeure), 0xFF888888.toInt(),
    ...
    private suspend fun fetchNextPassage(stopName: String, lineName: String): PassageInfo? {
        val stop = AppData.busStops.find { it.name == stopName } ?: return null
        if (stop.codes.isEmpty()) return gtfsFallback(stop, lineName)
        return try {
            val infos = IdelisApi.getStopMonitoring(stop.codes.first(), 5)
            val info  = infos.find { it.ligne == lineName }
            if (info != null) {
                val first = info.passages.firstOrNull() ?: return gtfsFallback(stop, lineName)
                ...
                PassageInfo(first.arrivee, statut,
                    ...
                    info.destination)
            } else gtfsFallback(stop, lineName)
        } catch (_: Exception) { gtfsFallback(stop, lineName) }
    }

    private suspend fun gtfsFallback(stop: BusStop, lineName: String): PassageInfo? {
        return try {
            val infos = GtfsReader.getTheoreticalPassages(this, stop.codes,
                java.time.LocalTime.now(), java.time.LocalDate.now())
            val first = infos.find { it.ligne == lineName }?.passages?.firstOrNull() ?: return null
